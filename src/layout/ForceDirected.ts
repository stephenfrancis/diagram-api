...
class Node {
  ...
  public calcAttraction(sample: boolean): void {
    this.edges.forEach((other_node: Node) => {
      const v: Geom.Vector = this.calcAttractionForce(other_node,
  ...
  public calcAttractionForce(other_node: Node, spring_length: number): Geom.Vector {
    const between: Geom.Vector = this.getVectorTo(other_node);
    const proximity: number = Math.max(between.getMagnitude(), 1000);
    return new Geom.Vector(this.fd.getAttractionConstant()
  ...
  public calcRepulsionForce(other_node: Node): Geom.Vector {
    const between: Geom.Vector = this.getVectorTo(other_node);
    const proximity: number = Math.max(between.getMagnitude(), 1000);
    return new Geom.Vector(-(this.fd.getRepulsionConstant() / Math.pow(proximity, 2)),
      between.getBearing());
  }
  ...
  public reset(): void {
    this.position = new Geom.Point((0.1 + Math.random()) * 800, (0.1 + Math.random()) * 500);
