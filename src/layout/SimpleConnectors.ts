...
export default class SimpleConnectors {
  ...
  public doLevel2(connector: Connector): void {
    const v: Geom.Vector = Geom.Vector.between(connector.getTo().getCentre(), connector.getFrom().getCentre());
    const   to_dir: Geom.Direction = Geom.Direction.nearest(v.getBearing());
    const from_dir: Geom.Direction = Geom.Direction.nearest(180 + v.getBearing());
  ...
  public doLevel4(connector: Connector): void {
    ...
    const to_dir: Geom.Direction = connector.getToDirection();
    const to_anchor: Geom.Point = connector.getTo().getAnchorPoint(to_dir);
    const from_shift: Geom.Point = connector.shift(from_anchor, from_dir, to_anchor);
    const to_shift: Geom.Point = connector.shift(to_anchor, to_dir, from_anchor);
    const elbow: Geom.Point = new Geom.Point(from_shift.getX(), to_shift.getY());
  ...
  public layoutDomain(Domain: Domain): void {
    console.log(`SimpleConnectors.layoutDomain() sophistication: ${this.sophistication}`);
